...
def test_walkforward_can_select_between_breakout_and_reversion_families() -> None:
    candidates = build_breakout_parameter_grid(
        strategy_kinds=["breakout", "reversion"],
        lookbacks=[60],
        hold_seconds=[300],
    ...
    assert {candidate.strategy_kind for candidate in candidates} == {"breakout", "reversion"}
    ...
    def evaluator(candidate, start_date: str, end_date: str) -> BacktestReport:
        ...
        if candidate.strategy_kind == "reversion":
            return _report(net_pnl="2", drawdown="1")
    ...
    assert report.folds[0].selected_candidate.strategy_kind == "reversion"
    assert report.selected_parameter_value_counts["strategy_kind"] == {"reversion": 2}
