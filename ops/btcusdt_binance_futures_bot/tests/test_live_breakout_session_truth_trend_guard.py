...
def test_live_breakout_session_truth_trend_observe_only_blocks_entries(tmp_path) -> None:
    guard_path = tmp_path / "session_truth_trend_guard.json"
    ...
    with JSONLWriter(tmp_path) as writer:
        runner = LiveBreakoutRunner(
            ...
            live_config=LiveBreakoutConfig(
                ...
                session_truth_trend_guard_path=str(guard_path),
        ...
        assert runner.status.session_truth_trend_observe_rejections == 1
        assert runner.status.last_gate_reason == "session_truth_trend_observe_only"
