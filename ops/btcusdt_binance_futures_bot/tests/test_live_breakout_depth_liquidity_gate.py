...
def test_live_breakout_depth_liquidity_gate_blocks_thin_exit_book(tmp_path) -> None:
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.status.depth_liquidity_gate_rejections == 1
        assert runner.status.last_gate_reason == "insufficient_exit_depth_coverage"
        assert Decimal(runner.status.last_exit_depth_coverage_ratio) < Decimal("0.75")
