...
def test_live_breakout_queue_gate_blocks_when_fill_is_unlikely(tmp_path) -> None:
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.status.queue_gate_rejections == 1
        assert runner.status.last_gate_reason == "queue_ahead_too_large"
