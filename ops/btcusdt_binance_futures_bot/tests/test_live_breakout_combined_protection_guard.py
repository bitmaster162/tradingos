...
def test_live_breakout_combined_guard_observe_only_blocks_entries(tmp_path) -> None:
    guard_path = tmp_path / "combined_guard.json"
    guard_path.write_text(
        json.dumps({"action": "observe_only", "size_multiplier": "0", "cooldown_until_ms": 1_700_000_100_000}),
        encoding="utf-8",
    )
    ...
    with JSONLWriter(tmp_path) as writer:
        runner = LiveBreakoutRunner(
            ...
            live_config=LiveBreakoutConfig(
                ...
                combined_protection_guard_path=str(guard_path),
        ...
        assert runner.status.combined_protection_observe_rejections == 1
        assert runner.status.last_gate_reason == "combined_protection_observe_only"


def test_live_breakout_combined_guard_reduce_size_scales_entry_qty(tmp_path) -> None:
    guard_path = tmp_path / "combined_guard.json"
    guard_path.write_text(
        json.dumps({"action": "reduce_size", "size_multiplier": "0.5", "cooldown_until_ms": 0}),
        encoding="utf-8",
    )
    config = _build_config(tmp_path)
    gateway = FakeGateway()
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.status.combined_protection_reduce_size_applications == 1
        assert runner.status.last_combined_protection_action == "reduce_size"
        assert runner.status.last_combined_protection_size_multiplier == "0.5"
