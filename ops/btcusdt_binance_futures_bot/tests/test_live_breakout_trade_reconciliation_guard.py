...
def test_live_breakout_trade_reconciliation_observe_only_blocks_entries(tmp_path) -> None:
    guard_path = tmp_path / "trade_reconciliation_guard.json"
    ...
    with JSONLWriter(tmp_path) as writer:
        runner = LiveBreakoutRunner(
            ...
            live_config=LiveBreakoutConfig(
                ...
                trade_reconciliation_guard_path=str(guard_path),
        ...
        assert runner.status.trade_reconciliation_observe_rejections == 1
        assert runner.status.last_gate_reason == "trade_reconciliation_observe_only"


def test_live_breakout_trade_reconciliation_reduce_size_scales_entry_qty(tmp_path) -> None:
    guard_path = tmp_path / "trade_reconciliation_guard.json"
    guard_path.write_text(
        json.dumps({
            ...
            "size_multiplier": "0.5",
            "window_mode": "session",
            "session_started_at_ms": 1700000000000,
            "missing_local_trade_ratio": "0.15",
            "missing_local_order_ratio": "0.20",
            "realized_pnl_diff_usdt": "2.0",
            "commission_abs_diff_usdt": "0.3",
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.status.trade_reconciliation_reduce_size_applications == 1
        assert runner.status.last_trade_reconciliation_action == "reduce_size"
        assert runner.status.last_trade_reconciliation_size_multiplier == "0.5"
        assert runner.status.last_trade_reconciliation_window_mode == "session"
        assert runner.status.last_trade_reconciliation_session_started_at_ms == 1700000000000
        assert runner.status.last_trade_reconciliation_missing_local_order_ratio == "0.20"
        assert runner.status.last_trade_reconciliation_quote_qty_abs_diff_usdt == "12.5"
        assert runner.status.last_trade_reconciliation_income_trade_link_gap_ratio == "0.25"
