...
def _book(event_time_ms: int, bid: str, ask: str) -> BacktestEvent:
    return BacktestEvent(
        event_time_ms=event_time_ms,
        stream="btcusdt@bookTicker",
        event_type="bookTicker",
        payload={"e": "bookTicker", "E": event_time_ms, "b": bid, "B": "1", "a": ask, "A": "1"},
...
def test_parity_backtester_blocks_entry_when_book_spread_is_too_wide() -> None:
    events = [
        _book(900, "99", "103"),
    ...
    backtester = ParityBreakoutBacktester(
        ...
        config=BreakoutBacktestConfig(
            ...
            max_book_spread_bps=Decimal("50"),
            use_book_ticker_fills=True,
        ),
        filters=FILTERS,
    )
    ...
    assert report.book_gate_rejections == 1
    assert report.last_book_spread_bps is not None
