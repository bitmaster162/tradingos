...
def test_live_breakout_session_truth_observe_only_blocks_entries(tmp_path) -> None:
    guard_path = tmp_path / "session_truth_guard.json"
    ...
    with JSONLWriter(tmp_path) as writer:
        runner = LiveBreakoutRunner(
            ...
            live_config=LiveBreakoutConfig(
                ...
                session_truth_guard_path=str(guard_path),
        ...
        assert runner.status.session_truth_observe_rejections == 1
        assert runner.status.last_gate_reason == "session_truth_observe_only"



def test_live_breakout_session_truth_reduce_size_scales_entry_qty(tmp_path) -> None:
    guard_path = tmp_path / "session_truth_guard.json"
    guard_path.write_text(
        json.dumps({
            ...
            "net_realized_pnl_usdt": "-1.5",
            "net_realized_bps": "-2.5",
            "maker_ratio": "0.30",
        }),
        encoding="utf-8",
    )
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.status.session_truth_reduce_size_applications == 1
        assert runner.status.last_session_truth_action == "reduce_size"
        assert runner.status.last_session_truth_size_multiplier == "0.5"
        assert runner.status.last_session_truth_window_mode == "session"
        assert runner.status.last_session_truth_session_started_at_ms == 1700000000000
        assert runner.status.last_session_truth_net_realized_bps == "-2.5"
        assert runner.status.last_session_truth_maker_ratio == "0.30"
