...
def test_live_breakout_runner_supports_ensemble_strategy_kind(tmp_path) -> None:
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.model.strategy_kind == "ensemble"
        assert runner.status.strategy_kind == "ensemble"
        assert runner.status.ensemble_reversion_signal_count == 1
        assert runner.status.last_ensemble_selected_strategy_kind == "reversion"
