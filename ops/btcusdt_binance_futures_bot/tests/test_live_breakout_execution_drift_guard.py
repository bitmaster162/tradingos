...
def test_live_breakout_observe_only_guard_blocks_entries(tmp_path) -> None:
    ...
    with JSONLWriter(tmp_path) as writer:
        runner = LiveBreakoutRunner(
            ...
            live_config=LiveBreakoutConfig(
                ...
                execution_drift_guard_path=str(guard_path),
        ...
        assert runner.status.execution_drift_observe_rejections == 1
        assert runner.status.last_gate_reason == "execution_drift_observe_only"


def test_live_breakout_reduce_size_guard_scales_entry_qty(tmp_path) -> None:
    guard_path = tmp_path / "guard.json"
    guard_path.write_text(json.dumps({"action": "reduce_size", "size_multiplier": "0.5"}), encoding="utf-8")
    config = _build_config(tmp_path)
    gateway = FakeGateway()
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert runner.status.execution_drift_reduce_size_applications == 1
        assert runner.status.last_execution_drift_action == "reduce_size"
        assert runner.status.last_execution_drift_size_multiplier == "0.5"
