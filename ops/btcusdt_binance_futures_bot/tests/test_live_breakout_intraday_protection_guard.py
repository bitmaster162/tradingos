...
def test_live_breakout_intraday_observe_only_blocks_entries(tmp_path) -> None:
    guard_path = tmp_path / "intraday_guard.json"
    ...
    with JSONLWriter(tmp_path) as writer:
        runner = LiveBreakoutRunner(
            ...
            live_config=LiveBreakoutConfig(
                ...
                intraday_protection_guard_path=str(guard_path),
        ...
        assert runner.status.intraday_protection_observe_rejections == 1
        assert runner.status.last_gate_reason == "intraday_protection_observe_only"


def test_live_breakout_intraday_reduce_size_scales_entry_qty(tmp_path) -> None:
    guard_path = tmp_path / "intraday_guard.json"
    guard_path.write_text(json.dumps({"action": "reduce_size", "size_multiplier": "0.6"}), encoding="utf-8")
    config = _build_config(tmp_path)
    gateway = FakeGateway()
    ...
    with JSONLWriter(tmp_path) as writer:
        ...
        assert proposal.qty == Decimal("96") / Decimal("101")
        assert runner.status.last_target_notional_usdt == "96.000"
        assert runner.status.intraday_protection_reduce_size_applications == 1
        assert runner.status.last_intraday_protection_action == "reduce_size"
        assert runner.status.last_intraday_protection_size_multiplier == "0.6"
