...
def test_walkforward_can_select_router_strategy_family() -> None:
    candidates = build_breakout_parameter_grid(
        strategy_kinds=["breakout", "reversion", "router"],
        lookbacks=[60],
        hold_seconds=[300],
    ...
    assert {candidate.strategy_kind for candidate in candidates} == {"breakout", "reversion", "router"}
    ...
    def evaluator(candidate, start_date: str, end_date: str) -> BacktestReport:
        ...
        if candidate.strategy_kind == "router":
            return _report(net_pnl="3", drawdown="1")
    ...
    assert report.folds[0].selected_candidate.strategy_kind == "router"
    assert report.selected_parameter_value_counts["strategy_kind"] == {"router": 2}
