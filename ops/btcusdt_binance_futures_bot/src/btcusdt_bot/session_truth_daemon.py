...
from btcusdt_bot.monitoring.session_truth import (
    SessionTruthDecision,
    SessionTruthStatus,
    SessionTruthThresholds,
    evaluate_session_truth,
)
...
class SessionTruthDaemonConfig:
    lookback_ms: int
...
class SessionTruthDaemon:
    def __init__(
        ...
        *,
        ...
        self.logger = logger or logging.getLogger("btcusdt_bot.session_truth")
        self.status = SessionTruthStatus()
        self.archive = (
    ...
    def _evaluate(
        self,
    ) -> tuple[SessionTruthDecision, list[dict[str, object]], list[dict[str, object]], dict[str, object]]:
        ...
        return decision, user_trades, income_rows, source_meta
    ...
    async def run(
        ...
        *,
        ...
        while True:
            ...
            try:
                decision, user_trades, income_rows, source_meta = await asyncio.to_thread(self._evaluate)
                latest_path = self.writer.write_json("live/guards/latest_session_truth.json", decision)
                session_truth_report = build_session_truth_report(
                    exchange_user_trades=user_trades,
                    exchange_income_rows=income_rows,
                    lookback_start_ms=decision.lookback_start_ms,
                    lookback_end_ms=decision.lookback_end_ms,
                    bucket_ms=60 * 60 * 1000,
                    compared_at_ms=decision.compared_at_ms,
                    window_mode=decision.window_mode,
                    session_started_at_ms=decision.session_started_at_ms,
                )
                latest_report_path = self.writer.write_json(
                    "live/reports/latest_session_truth.json",
                    {
                        "decision": decision,
                        "source": source_meta,
                        "exchange_user_trade_count": len(user_trades),
                        "exchange_income_row_count": len(income_rows),
                    },
                )
                latest_session_truth_report_path = self.writer.write_json(
                    "live/reports/latest_session_truth_report.json",
                    {
                        "report": session_truth_report,
                        "source": source_meta,
                    },
                )
                report_path = self.writer.append_record(
                    "reports",
                    f"{self.config.symbol.lower()}_session_truth",
                    {
                        "decision": decision,
                        "source": source_meta,
                        "exchange_user_trade_count": len(user_trades),
                ...
                self.writer.append_record(
                    "reports",
                    f"{self.config.symbol.lower()}_session_truth_report",
                    {
                        "report": session_truth_report,
                        "source": source_meta,
                    },
                ...
                self.status.maker_ratio = decision.maker_ratio
                self.status.net_realized_pnl_usdt = decision.net_realized_pnl_usdt
                self.status.net_realized_bps = decision.net_realized_bps
                self.status.commission_bps = decision.commission_bps
                self.status.funding_bps = decision.funding_bps
                ...
                self.writer.write_json("live/guards/latest_session_truth_status.json", self.status)
                self.logger.info(
                    "session truth decision=%s window_mode=%s source_mode=%s latest=%s live_report=%s trend_report=%s",
                    decision.action,
                    decision.window_mode,
                    self.status.last_source_mode,
                    latest_path,
                    latest_report_path,
                    latest_session_truth_report_path,
                )
            except Exception as exc:  # noqa: BLE001
                self.status.last_error = str(exc)
                self.writer.write_json("live/guards/latest_session_truth_status.json", self.status)
                self.logger.warning("session truth evaluation failed: %s", self.status.last_error)
