...
$LoopScript = Join-Path $Root "ops\autostart\Run-BybitAllLiquidationWatchdogLoop.ps1"
$LoopLockPath = Join-Path $LogDir "bybit_all_liquidation_watchdog_loop.lock.json"
$LoopStatusPath = Join-Path $LogDir "bybit_all_liquidation_watchdog_loop_status.json"
...
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "bybit_all_liquidation_watchdog" -AttemptId $LaunchAttemptId
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "bybit_all_liquidation_watchdog" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "bybit_all_liquidation_watchdog" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "bybit_all_liquidation_watchdog" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: bybit_all_liquidation_watchdog"
}
