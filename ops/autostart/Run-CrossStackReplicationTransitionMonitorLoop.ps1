...
$StdoutPath = Join-Path $LogDir "cross_stack_replication_transition_loop_stdout.log"
$StderrPath = Join-Path $LogDir "cross_stack_replication_transition_loop_stderr.log"
...
if (Test-Path -LiteralPath $LoopLockPath) {
    try {
        ...
        if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
            Write-LoopStatus -Status "skipped_existing_cross_stack_replication_transition_loop" -Extra @{ existing_pid = $ExistingPid }
            exit 0
        }
    } catch {}
...
$Args += @(
    ...
    "--out-prefix", "docs\CROSS_STACK_REPLICATION_TRANSITION_MONITOR_2026-07-03"
)
...
try {
    while ($true) {
        ...
        Push-Location $Root
        try {
            Write-LoopStatus -Status "running_transition_monitor_cycle" -Extra @{ python = $Python.Exe }
            & $Python.Exe @Args >> $StdoutPath 2>> $StderrPath
            $ExitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } else { 0 }
            Write-LoopStatus -Status "ran_transition_monitor_cycle" -ExitCode $ExitCode -Extra @{ python = $Python.Exe }
        } finally {
            Pop-Location
        }
        Start-Sleep -Seconds $SleepSeconds
