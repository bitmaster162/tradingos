...
$LogDir = Join-Path $Root "logs\cross_stack_replication"
$LoopScript = Join-Path $Root "ops\autostart\Run-CrossStackReplicationTransitionMonitorLoop.ps1"
$LoopLockPath = Join-Path $LogDir "cross_stack_replication_transition_loop.lock.json"
$LoopStatusPath = Join-Path $LogDir "cross_stack_replication_transition_loop_status.json"
...
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "cross_stack_replication_transition" -AttemptId $LaunchAttemptId
...
if (-not $LaunchDisposition.should_start) {
    ...
    [ordered]@{
        ...
        external_replication_monitor_only = $true
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "cross_stack_replication_transition" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "cross_stack_replication_transition" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "cross_stack_replication_transition" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: cross_stack_replication_transition"
}
