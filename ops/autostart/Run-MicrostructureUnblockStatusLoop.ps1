...
$StdoutPath = Join-Path $LogDir "microstructure_unblock_status_stdout.log"
$StderrPath = Join-Path $LogDir "microstructure_unblock_status_stderr.log"
...
if (Test-Path -LiteralPath $LoopLockPath) {
    try {
        ...
        if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
            Write-LoopStatus -Status "skipped_existing_microstructure_unblock_status_loop" -Extra @{ existing_pid = $ExistingPid }
            exit 0
        }
    } catch {}
...
$Args += @(
    ...
    "--refresh",
    "--out-prefix", "docs\MICROSTRUCTURE_UNBLOCK_STATUS_2026-07-03"
)
