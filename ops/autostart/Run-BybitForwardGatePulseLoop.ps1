...
$StdoutPath = Join-Path $LogDir "bybit_forward_gate_pulse_loop_stdout.log"
$StderrPath = Join-Path $LogDir "bybit_forward_gate_pulse_loop_stderr.log"
...
if (Test-Path -LiteralPath $LoopLockPath) {
    try {
        ...
        if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
            Write-LoopStatus -Status "skipped_existing_bybit_forward_gate_pulse_loop" -Extra @{ existing_pid = $ExistingPid }
            exit 0
        }
    } catch {}
...
$Args += @(
    ...
    "--out-prefix", "docs\BYBIT_LIQUIDATION_FORWARD_GATE_PULSE_2026-07-03"
)
...
try {
    while ($true) {
        ...
        Push-Location $Root
        try {
            Write-LoopStatus -Status "running_pulse_cycle" -Extra @{ python = $Python.Exe }
            & $Python.Exe @Args >> $StdoutPath 2>> $StderrPath
            $ExitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } else { 0 }
            Write-LoopStatus -Status "ran_pulse_cycle" -ExitCode $ExitCode -Extra @{ python = $Python.Exe }
        } finally {
            Pop-Location
        }
        Start-Sleep -Seconds $SleepSeconds
