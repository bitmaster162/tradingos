...
$LoopScript = Join-Path $Root "ops\autostart\Run-MicrostructureUnblockStatusLoop.ps1"
$LoopLockPath = Join-Path $LogDir "microstructure_unblock_status_loop.lock.json"
$LoopStatusPath = Join-Path $LogDir "microstructure_unblock_status_loop_status.json"
...
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "microstructure_unblock_status" -AttemptId $LaunchAttemptId
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "microstructure_unblock_status" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "microstructure_unblock_status" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "microstructure_unblock_status" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: microstructure_unblock_status"
}
