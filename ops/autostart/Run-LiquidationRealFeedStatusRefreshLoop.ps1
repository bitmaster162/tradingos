...
$StdoutPath = Join-Path $LogDir "liquidation_real_feed_status_refresh_stdout.log"
$StderrPath = Join-Path $LogDir "liquidation_real_feed_status_refresh_stderr.log"
...
if (Test-Path -LiteralPath $LoopLockPath) {
    try {
        ...
        if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
            Write-LoopStatus -Status "skipped_existing_liquidation_real_feed_status_refresh_loop" -Extra @{ existing_pid = $ExistingPid }
            exit 0
        }
    } catch {}
...
$Args += @(
    ...
    "--refresh-collectors",
    "--refresh-price-tail",
    "--out-prefix", "docs\LIQUIDATION_REAL_FEED_STATUS_REFRESH_2026-07-02_FORWARD_OBSERVER"
)
...
try {
    while ($true) {
        ...
        Push-Location $Root
        try {
            Write-LoopStatus -Status "running_refresh_cycle" -Extra @{ python = $Python.Exe }
            & $Python.Exe @Args >> $StdoutPath 2>> $StderrPath
            $ExitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } else { 0 }
            Write-LoopStatus -Status "ran_refresh_cycle" -ExitCode $ExitCode -Extra @{ python = $Python.Exe }
        } finally {
            Pop-Location
        }
        Start-Sleep -Seconds $SleepSeconds
