...
$LogDir = Join-Path $Root "logs\liquidation_real_feed"
$LoopScript = Join-Path $Root "ops\autostart\Run-LiquidationRealFeedStatusRefreshLoop.ps1"
$LoopLockPath = Join-Path $LogDir "liquidation_real_feed_status_refresh_loop.lock.json"
$LoopStatusPath = Join-Path $LogDir "liquidation_real_feed_status_refresh_loop_status.json"
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
$RuntimeManifest = Get-TradingOSRuntimeManifest -Root $Root
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "liquidation_real_feed_status_refresh" -AttemptId $LaunchAttemptId
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "liquidation_real_feed_status_refresh" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "liquidation_real_feed_status_refresh" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "liquidation_real_feed_status_refresh" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: liquidation_real_feed_status_refresh"
}
