...
$LoopScript = Join-Path $Root "ops\autostart\Run-RealEdgeObserverPulseLoop.ps1"
...
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "real_edge_observer" -AttemptId $LaunchAttemptId
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "real_edge_observer" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "real_edge_observer" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "real_edge_observer" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: real_edge_observer"
}
