...
$LoopScript = Join-Path $Root "ops\autostart\Run-CrossVenueMicrostructureBookLoop.ps1"
$LoopLockPath = Join-Path $LogDir "microstructure_book_loop.lock.json"
$LoopStatusPath = Join-Path $LogDir "microstructure_book_loop_status.json"
$ProcessStdoutPath = Join-Path $LogDir "microstructure_book_process_stdout.log"
$ProcessStderrPath = Join-Path $LogDir "microstructure_book_process_stderr.log"
...
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "cross_venue_microstructure_book" -AttemptId $LaunchAttemptId
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "cross_venue_microstructure_book" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript -StdoutPath $ProcessStdoutPath -StderrPath $ProcessStderrPath
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "cross_venue_microstructure_book" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "cross_venue_microstructure_book" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: cross_venue_microstructure_book"
}
