...
$LoopScript = Join-Path $Root "ops\autostart\Run-BybitForwardGatePulseLoop.ps1"
$LoopLockPath = Join-Path $LogDir "bybit_forward_gate_pulse_loop.lock.json"
$LoopStatusPath = Join-Path $LogDir "bybit_forward_gate_pulse_loop_status.json"
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null
$RuntimeManifest = Get-TradingOSRuntimeManifest -Root $Root
$LaunchDisposition = Get-TradingOSRuntimeLaunchDisposition -Root $Root -Manifest $RuntimeManifest -ComponentId "bybit_forward_gate_pulse" -AttemptId $LaunchAttemptId
...
try {
    $Process = Start-TradingOSRuntimeJobProcess -Root $Root -ComponentId "bybit_forward_gate_pulse" -AttemptId $LaunchAttemptId -FilePath "powershell.exe" -ArgumentList $Args -WorkingDirectory $Root -ExpectedScriptPath $LoopScript
    ...
    do {
        ...
        if ($StatusPidAfter -eq $Process.Id) {
            try {
                $Confirmation = Get-TradingOSRuntimeComponentLaunchConfirmation -Root $Root -Manifest $RuntimeManifest -ComponentId "bybit_forward_gate_pulse" -AttemptId $LaunchAttemptId -ExpectedProcessId $Process.Id -StatusProcessId $StatusPidAfter
                $ConfirmationError = $null
...
if (-not $Confirmed) {
    $Rollback = $null
    $RollbackError = $null
    try { $Rollback = Undo-TradingOSRuntimeComponentLaunch -Root $Root -AttemptId $LaunchAttemptId -ComponentId "bybit_forward_gate_pulse" } catch { $RollbackError = $_.Exception.Message }
    [ordered]@{
    ...
    throw "Runtime component launch failed or was not confirmed: bybit_forward_gate_pulse"
}
