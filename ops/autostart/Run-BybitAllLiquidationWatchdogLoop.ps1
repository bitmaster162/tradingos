...
$StdoutPath = Join-Path $LogDir "bybit_all_liquidation_watchdog_stdout.log"
$StderrPath = Join-Path $LogDir "bybit_all_liquidation_watchdog_stderr.log"
...
if (Test-Path -LiteralPath $LoopLockPath) {
    try {
        ...
        if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
            Write-LoopStatus -Status "skipped_existing_bybit_all_liquidation_watchdog_loop" -Extra @{ existing_pid = $ExistingPid }
            exit 0
        }
    } catch {}
...
$Args += @(
    ...
    "--out-prefix", "docs\BYBIT_ALL_LIQUIDATION_COLLECTOR_WATCHDOG_2026-07-01",
    "--sample-gate-prefix", "docs\BYBIT_ALL_LIQUIDATION_SAMPLE_GATE_2026-07-01"
)
...
try {
    while ($true) {
        ...
        Push-Location $Root
        try {
            Write-LoopStatus -Status "running_watchdog_cycle" -Extra @{ python = $Python.Exe }
            & $Python.Exe @Args >> $StdoutPath 2>> $StderrPath
            $ExitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } else { 0 }
            Write-LoopStatus -Status "ran_watchdog_cycle" -ExitCode $ExitCode -Extra @{ python = $Python.Exe }
        } finally {
            Pop-Location
        }
        Start-Sleep -Seconds $SleepSeconds
