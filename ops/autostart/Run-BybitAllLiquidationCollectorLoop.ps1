...
$StdoutPath = Join-Path $LogDir "bybit_all_liquidation_stdout.log"
$StderrPath = Join-Path $LogDir "bybit_all_liquidation_stderr.log"
...
if (Test-Path -LiteralPath $LoopLockPath) {
    try {
        ...
        if ($ExistingPid -gt 0 -and (Get-Process -Id $ExistingPid -ErrorAction SilentlyContinue)) {
            Write-LoopStatus -Status "skipped_existing_bybit_all_liquidation_loop" -Extra @{ existing_pid = $ExistingPid }
            exit 0
        }
    } catch {
...
$Args += $Python.Prefix
$Args += @(
    "tools\bybit_all_liquidation_real_feed_collector_v2.py",
    "--symbols", $Symbols,
    "--max-events", "$MaxEventsPerCycle",
    "--max-seconds", "$CycleSeconds",
    "--out-prefix", "docs\BYBIT_ALL_LIQUIDATION_FORWARD_COLLECTOR_LATEST"
)

Write-LoopStatus -Status "running" -Extra @{ python = $Python.Exe; ingest_schema_version = 4; packet_atomic_write = $true }
...
try {
    while ($true) {
        ...
        Push-Location $Root
        try {
            Write-LoopStatus -Status "running_collector_cycle" -Extra @{ python = $Python.Exe; ingest_schema_version = 4; packet_atomic_write = $true }
