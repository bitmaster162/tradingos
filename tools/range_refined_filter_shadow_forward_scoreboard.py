...
from tools.range_refined_forward_observer import selected_candidate  # noqa: E402
from tools.range_refined_observer_scoreboard import load_bars, max_drawdown, max_losing_streak, parse_rr, resolve_outcome  # noqa: E402
...
def dedupe_variant_events(events: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    out: dict[str, dict[str, Any]] = {}
    for event in events:
        if event.get("event_type") != "range_filter_shadow_variant_state":
            continue
        variant_id = str(event.get("variant_id") or "")
        bar_ts = str(event.get("bar_ts") or "")
        if not variant_id or not bar_ts:
            continue
        key = f"{variant_id}|{bar_ts}"
        out[key] = event
    return out


def synthetic_signal_event(event: dict[str, Any], selected: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "side": event.get("side") or selected.get("side"),
        "atr": atr,
        "rr": selected.get("rr"),
...
def summarize_outcomes(
    *,
    ...
    if signal_events == 0:
        ...
        classification = "shadow_watchlist_positive_insufficient_sample" if expectancy is not None and expectancy >= min_expectancy_r else "shadow_insufficient_forward_sample"
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for key, event in sorted(unique_events.items(), key=lambda item: str(item[1].get("bar_ts") or "")):
        ...
        if status == "shadow_variant_signal_observed":
            ...
            signal_event = synthetic_signal_event(event, selected)
    ...
    variant_rows = []
    for variant_id, bucket in sorted(variants.items()):
    ...
    return {
        ...
        "runtime_boundary": {
            "classification": "range_refined_filter_shadow_forward_scoreboard_public_cache_only",
            "can_trade": False,
            "sends_orders": False,
        ...
        "unique_variant_bar_events": len(unique_events),
        "selected_candidate": selected,
        "summary": overall,
        "variants": variant_rows,
        "outcomes": outcomes,
        "decision": "shadow_forward_scoreboard_no_trade_permission",
        "next_action": "accumulate shadow variant signals; do not promote without enough resolved forward outcomes and a separate gate",
        "can_trade": False,
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Boundary",
        "",
        "- Scores observer-only `shadow_variant_signal_observed` events by filter variant.",
        ...
        f"- Filtered / no-base: `{summary.get('shadow_filtered_out_events')}` / `{summary.get('shadow_no_base_events')}`.",
        ...
        "## Variants",
        "",
        "| Variant | Classification | Signals | Resolved | Winrate | Exp R | Historical Verdict | Cost +10 Exp |",
        "|---|---|---:|---:|---:|---:|---|---:|",
    ]
    for row in report.get("variants", []):
        ...
        lines.append(
            f"| `{row.get('variant_id')}` | `{row_summary.get('classification')}` | "
            ...
            f"`{evidence.get('verdict')}` | `{evidence.get('cost10_expectancy_r')}` |"
        )
    lines.extend(
        [
            "",
            "## Interpretation",
            "",
            "- Current no-signal state is useful evidence: it separates base-setup scarcity from filter scarcity.",
            "- A variant may enter review only after enough real forward shadow signals are resolved.",
            "- Historical ablation quality is not enough for promotion without forward evidence.",
            "",
            "## Files",
            "",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Score observer-only outcomes for RANGE filter shadow variants")
    ...
    parser.add_argument("--out-prefix", default="docs/RANGE_REFINED_FILTER_SHADOW_FORWARD_SCOREBOARD_2026-06-17")
