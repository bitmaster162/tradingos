...
def render_html(report: dict[str, Any]) -> str:
    ...
    for row in report["matrix"]:
        ...
        cards.append(
            f'<article><header><div><h2>{esc(row["symbol"])}</h2><strong class="{row["state"].lower()}">{row["state"]}</strong></div><em>ATTN <b>{row["attention_score"]:.0f}</b></em></header>'
...
def main() -> int:
    ...
    print(json.dumps({"result": "PASS", "top_attention": report["top_attention"], "outputs": {"json": str(jp), "html": str(hp)}, "can_trade": False, "capital_permission": "DENY"}, indent=2)); return 0
