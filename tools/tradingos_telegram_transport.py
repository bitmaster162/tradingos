...
def execute(a,pf,m,c,rp,ledger,at,mode,e,max_age=60,timeout=15,sender=http_send):
 ...
 try:
  outcome=sender('api.telegram.org',f'/bot{token}/sendMessage',body,timeout)
 ...
 ok=outcome.get('http_status')==200 and outcome.get('ok') is True
 state='DELIVERED' if ok else 'FAILED_NO_RETRY'
 try:
  post=append(ledger,{**common,'record_type':'NETWORK_OUTCOME','state':state,'network_call':True,'send_performed':ok,'transport_attempted':True,'http_status':outcome.get('http_status'),'telegram_ok':outcome.get('ok') is True,'telegram_message_id':outcome.get('message_id'),'telegram_error_code':outcome.get('error_code')})
 except Exception as exc:
  return {'status':'DELIVERED_LEDGER_UNCERTAIN_NO_RETRY' if ok else 'FAILED_LEDGER_UNCERTAIN_NO_RETRY','claim_file':cp.name,'prepared_record_hash':pre['record_hash'],'outcome_record_hash':None,'network_call':True,'send_performed':ok,'transport_attempted':True,'telegram_message_id':outcome.get('message_id') if ok else None,'manual_reconciliation_required':True,'outcome_persisted':False,'ledger_error_type':type(exc).__name__,'automatic_retry':False,'can_trade':False}
 return {'status':state,'claim_file':cp.name,'prepared_record_hash':pre['record_hash'],'outcome_record_hash':post['record_hash'],'network_call':True,'send_performed':ok,'transport_attempted':True,'telegram_message_id':outcome.get('message_id') if ok else None,'manual_reconciliation_required':False,'outcome_persisted':True,'automatic_retry':False,'can_trade':False}
