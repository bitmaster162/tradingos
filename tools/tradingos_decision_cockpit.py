...
def build(b:dict[str,Any],s:dict[str,Any],pb:dict[str,Any]|None=None,ps:dict[str,Any]|None=None)->dict[str,Any]:
    p=b.get("permissions",{})
    if b.get("can_trade") is not False or p.get("orders_allowed") is not False or p.get("signals_allowed") is not False:
        raise ValueError("unsafe execution permission")
    ...
    return {"schema":"tradingos.decision_cockpit.v1","version":VERSION,"brief_id":b.get("brief_id"),"symbol":b.get("symbol"),"timeframe":b.get("timeframe"),"as_of":b.get("as_of"),"status":b.get("status"),
      ...
      "quality":{"age":q.get("snapshot_age_minutes"),"missing":q.get("missing_data",[]),"conflicts":q.get("conflicts",[]),"blockers":q.get("blockers",[]),"sources":len(b.get("provenance",{}).get("input_sources",[]))},
