...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Real Edge Observer Pulse Loop Health",
        "",
    ...
    lines.extend(
        [
            ...
            f"- Transition decision: `{report.get('transition_decision')}`",
            f"- Waiting board decision: `{report.get('waiting_board_decision')}`",
            "",
            "## Boundary",
            "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Health audit for the Real Edge observer pulse loop.")
    parser.add_argument("--loop-status", default="logs/real_edge_observer/real_edge_observer_pulse_loop_status.json")
    parser.add_argument("--loop-lock", default="logs/real_edge_observer/real_edge_observer_pulse_loop.lock.json")
    parser.add_argument("--pulse-report", default="docs/REAL_EDGE_OBSERVER_PULSE_2026-07-03.json")
    parser.add_argument("--max-status-age-minutes", type=float, default=30.0)
    parser.add_argument("--out-prefix", default="docs/REAL_EDGE_OBSERVER_PULSE_LOOP_HEALTH_2026-07-03")
    ...
    sources = pulse.get("sources") if isinstance(pulse.get("sources"), dict) else {}
    checks = {
        "loop_status_readable": bool(loop),
        "loop_status_expected": loop.get("status") in {
            "running",
            "running_observer_pulse_cycle",
            "ran_observer_pulse_cycle",
            "sleeping_initial",
        },
        ...
        "telegram_send_not_allowed": loop.get("telegram_send_allowed") is False,
    ...
    report = {
        ...
        "tool": "tools/real_edge_observer_pulse_loop_health.py",
        "decision": "real_edge_observer_pulse_loop_healthy" if not failed else "real_edge_observer_pulse_loop_unhealthy",
        "can_trade": False,
        ...
        "transition_decision": (sources.get("transition_monitor") or {}).get("decision") if isinstance(sources.get("transition_monitor"), dict) else None,
        "waiting_board_decision": (sources.get("edge_waiting_board") or {}).get("decision") if isinstance(sources.get("edge_waiting_board"), dict) else None,
        "source_reports": {
