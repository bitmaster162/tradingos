...
r95 = _load("r96_sweep_r95", ROOT / "tools" / "r6_persistent_capacity_sweep.py")
cont = _load("r96_cont", ROOT / "tools" / "r6_decision_continuity_gate.py")
SCHEMA = "tradingos.r96_decision_continuity_sweep.v1"
VERSION = "R96_DECISION_CONTINUITY_SWEEP_V1_20260916"
...
async def run_sweep(symbols: list[str], runtime_state: dict[str, Any]) -> dict[str, Any]:
    ...
    for symbol in symbols:
        ...
        if continuity.get("status") != "PASS":
            ...
            results.append(item)
            continue
        final_candidate = _attach_continuity(candidate, continuity)
        event_id = final_candidate.get("event_id")
        if not isinstance(event_id, str) or not event_id:
            return fail("R96_EVENT_ID_INVALID", symbol=symbol)
        try:
            r95.r94._reserve_external(state, symbol, event_id)
        except Exception as exc:
            return fail("R96_RISK_RESERVATION_FAILED", symbol=symbol, error=str(exc))
        accepted.append(final_candidate)
        item.update(status="REGISTRATION_CANDIDATE_READY", stage="R96_DECISION_CONTINUITY",
                    registration_candidate=True, event_id=event_id,
                    decision_continuity_status="PASS")
        results.append(item)
    return {
