...
from tools import bybit_all_liquidation_real_feed_collector as collector
...
def validate_prereg(prereg: dict[str, Any]) -> list[str]:
    ...
    if int(prereg.get("schema_version") or 0) != 3:
    ...
    if int(receipt.get("required_ingest_schema_version") or 0) != 3:
        failures.append("receipt_contract.required_ingest_schema_version")
    if receipt.get("raw_wall_clock_is_arrival_order_evidence") is not False:
        failures.append("receipt_contract.raw_wall_clock_is_arrival_order_evidence")
    gate = prereg.get("input_quality_gate") if isinstance(prereg.get("input_quality_gate"), dict) else {}
    required_gate_fields = {
        "required_ingest_schema_version",
        ...
        "maximum_receipt_parse_errors",
        "maximum_correction_equation_failures",
        "maximum_calibration_age_failures",
        "maximum_calibration_rtt_failures",
        "maximum_calibration_uncertainty_failures",
        "maximum_clock_offset_failures",
        "maximum_calibration_sample_failures",
        "maximum_calibration_source_failures",
        "maximum_exchange_event_lag_failures",
        "maximum_corrected_before_event_failures",
        "maximum_corrected_receipt_lag_failures",
        "maximum_packet_receipt_conflicts",
        "maximum_packet_sequence_failures",
        "maximum_monotonic_receipt_failures",
        "minimum_clock_calibration_samples",
        "maximum_clock_calibration_rtt_ms",
        "maximum_clock_calibration_age_s",
        "maximum_absolute_clock_offset_ms",
        "maximum_exchange_event_lag_ms",
        "maximum_corrected_receipt_lag_ms",
        "corrected_receipt_uncertainty_grace_ms",
    ...
    tombstone = v2.base.resolve_path(str(prereg.get("data_quality_tombstone") or ""))
...
def build_lock(prereg_path: Path, *, created_at: str | None = None) -> dict[str, Any]:
    ...
    if failures:
        raise ValueError("invalid V4 preregistration: " + ",".join(failures))
    ...
    tombstone_path = v2.base.resolve_path(prereg["data_quality_tombstone"])
    ...
    return {
        ...
        "data_quality_tombstone": {"path": v2.base.portable(tombstone_path), "sha256": v2.sha256_file(tombstone_path)},
...
def validate_lock(lock: dict[str, Any]) -> list[str]:
    ...
    if int(receipt.get("required_ingest_schema_version") or 0) != 3:
        failures.append("receipt_contract")
...
def resolvable_metadata(
    ...
    indexes = {symbol: v2.study.build_bar_index(bars) for symbol, bars in closed_by_symbol.items()}
        ...
        metadata.append(
            {
                "symbol": symbol,
                "bar_ts": row["bar_ts"],
                "independent_4h_block": v2.study.independent_4h_block_id(row["bar_ts"]),
            }
        )
...
def render_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Bybit Canonical Liquidation Forward Observer V4",
            "",
            ...
            "- V1/V2/V3 observations admitted: `false`",
            "- Can trade: `false`",
            "",
            "## Blockers",
            "",
...
def run_observer(
    ...
    *,
    ...
    if failures:
        raise ValueError("invalid V4 forward lock: " + ",".join(failures))
    ...
    eligible_rows, closed_by_symbol, metadata, source_progress, quality_report = load_forward_progress(
        lock, now=observed_at
    )
    sample = v2.sample_summary(metadata)
    ...
    if observed_at < floor:
        decision = "bybit_liquidation_canonical_v4_waiting_floor"
        blockers = ["forward_floor_not_reached", *quality_blockers]
    elif quality_blockers:
        decision = "bybit_liquidation_canonical_v4_blocked_input_quality"
        ...
        records, record_errors = v2.study.build_event_records(
            eligible_rows,
            closed_by_symbol,
        ...
        if record_errors or len(records) != len(metadata):
            decision = "bybit_liquidation_canonical_v4_blocked_terminal_record_construction"
            blockers = ["terminal_record_construction_mismatch", *record_errors[:25]]
        else:
            base_decision, terminal_metrics, outcome_failures = v2.terminal_evaluation(
                records,
                lock["candidate"],
                lock["terminal_outcome_gate"],
            )
            decision = (
                "bybit_liquidation_canonical_v4_candidate_accepted_manual_shadow_only"
                if base_decision.endswith("accepted_manual_shadow_only")
                else "bybit_liquidation_canonical_v4_no_edge_tombstone"
            )
            blockers = outcome_failures
    terminal = decision in {
        "bybit_liquidation_canonical_v4_candidate_accepted_manual_shadow_only",
        "bybit_liquidation_canonical_v4_no_edge_tombstone",
    }
    next_action = (
        "keep the schema-v3 collector and V4 observer unchanged; do not compute or inspect interim returns"
        if not terminal
        else (
    ...
    report = {
        ...
        "tool": "tools/bybit_liquidation_canonical_forward_observer_v4.py",
        "decision": decision,
        "lock": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Calibrated-receipt, outcome-blind Bybit liquidation observer V4")
    subparsers = parser.add_subparsers(dest="command", required=True)
    seal = subparsers.add_parser("seal-lock")
    seal.add_argument("--prereg", default="configs/BYBIT_LIQUIDATION_CANONICAL_FORWARD_PREREG_V4_2026-07-14.json")
    seal.add_argument("--lock", default="configs/BYBIT_LIQUIDATION_CANONICAL_FORWARD_LOCK_V4_2026-07-14.json")
    seal.add_argument("--acknowledge-forward-only", action="store_true")
    run = subparsers.add_parser("run-once")
    run.add_argument("--lock", default="configs/BYBIT_LIQUIDATION_CANONICAL_FORWARD_LOCK_V4_2026-07-14.json")
    run.add_argument("--out-prefix", default="docs/BYBIT_LIQUIDATION_CANONICAL_FORWARD_OBSERVER_V4_2026-07-14")
    run.add_argument("--terminal-receipt", default="logs/bybit_liquidation_canonical_forward_v4/terminal_receipt.json")
    ...
    try:
        if args.command == "seal-lock":
            ...
            print(
                json.dumps(
                    {"decision": "bybit_canonical_v4_forward_lock_sealed", "lock": v2.base.portable(lock_path), "can_trade": False},
                    indent=2,
                )
            )
            return 0
        ...
        print(json.dumps({"decision": "bybit_canonical_v4_observer_error", "error": str(exc), "can_trade": False}, indent=2))
        return 2
    print(
        json.dumps(
            {
