...
def sample_brief() -> dict:
    return {
        ...
        "provenance":{"input_sources":[1,2,3,4],"generator":"tools/tradingos_decision_brief_v2.py","generator_version":"2.0.0","generator_sha256":"a"*64,"base_generator":"tools/tradingos_decision_brief.py","base_generator_sha256":"b"*64},
        "permissions":{"read_only_analysis":True,"signals_allowed":False,"orders_allowed":False,"uses_credentials":False,"can_trade":False,"capital_permission":"DENY"},
    }
...
def test_unsafe_brief_fails_closed() -> None:
    ...
    for key, unsafe in cases:
        brief = sample_brief()
        brief["permissions"][key] = unsafe
        try:
            cockpit.build_report(brief, sample_snapshot())
        except ValueError as exc:
            assert "unsafe" in str(exc)
        else:
            raise AssertionError(f"unsafe execution permission accepted: {key}={unsafe!r}")

    brief = sample_brief()
    brief["can_trade"] = True
    try:
        cockpit.build_report(brief, sample_snapshot())
    except ValueError as exc:
        assert "unsafe" in str(exc)
    else:
        raise AssertionError("unsafe top-level can_trade accepted")


def test_canonical_v2_provenance_is_preserved_without_becoming_a_gate() -> None:
    brief = sample_brief()
    provenance = dict(brief["provenance"])
    report = cockpit.build_report(brief, sample_snapshot())
    assert report["source_brief_provenance"] == provenance
    assert report["source_brief_provenance"]["generator"] == "tools/tradingos_decision_brief_v2.py"
    assert report["source_brief_provenance"]["generator_version"] == "2.0.0"
    assert report["quality"]["sources"] == 4

    brief["provenance"]["generator"] = "future-compatible-generator.py"
    report = cockpit.build_report(brief, sample_snapshot())
    assert report["source_brief_provenance"]["generator"] == "future-compatible-generator.py"


def test_non_object_provenance_fails_closed() -> None:
    brief = sample_brief()
    brief["provenance"] = []
    try:
        cockpit.build_report(brief, sample_snapshot())
        ...
        raise AssertionError("non-object provenance accepted")


def test_packet_identity_mismatch_fails_closed() -> None:
    for key, bad in (
        ("snapshot_id", "other"),
        ("symbol", "ETHUSDT"),
        ("timeframe", "1h"),
        ("as_of", "2026-08-09T20:00:00Z"),
    ):
        snapshot = sample_snapshot()
        snapshot[key] = bad
        try:
            cockpit.build_report(sample_brief(), snapshot)
        except ValueError as exc:
            assert "identity mismatch" in str(exc)
        else:
            raise AssertionError(f"mismatched packet identity accepted: {key}")


def test_unsafe_snapshot_and_partial_previous_packet_fail_closed() -> None:
    snapshot = sample_snapshot()
    snapshot["can_trade"] = True
    try:
        cockpit.build_report(sample_brief(), snapshot)
    except ValueError as exc:
        assert "unsafe" in str(exc)
    ...
    try:
        cockpit.build_report(sample_brief(), sample_snapshot(), sample_brief(), None)
    except ValueError as exc:
        assert "provided together" in str(exc)
    else:
        raise AssertionError("partial previous packet accepted")


def test_previous_packet_identity_is_independently_bound() -> None:
    prior_brief = sample_brief()
    prior_snapshot = sample_snapshot()
    prior_brief["snapshot_id"] = "prior"
    prior_snapshot["snapshot_id"] = "prior"
    prior_brief["as_of"] = "2026-08-09T12:00:00Z"
    prior_snapshot["as_of"] = "2026-08-09T12:00:00Z"

    report = cockpit.build_report(sample_brief(), sample_snapshot(), prior_brief, prior_snapshot)
    assert report["delta"]["state"] == "COMPARABLE"

    prior_snapshot["snapshot_id"] = "wrong"
    try:
        cockpit.build_report(sample_brief(), sample_snapshot(), prior_brief, prior_snapshot)
    except ValueError as exc:
        assert "previous packet identity mismatch" in str(exc)
    else:
        raise AssertionError("mismatched previous packet accepted")
