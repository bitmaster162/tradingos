...
class UnifiedShadowTransactionTests(unittest.TestCase):
    def setUp(self):
        ...
        thesis = build_trade_thesis(self.case, {"schema": "tradingos.decision_brief.v2", "status": "READY", "stance": "WATCH_LONG"})
        ...
        self.route = build_trade_case_route(case_id=self.case["case_id"], case_sha256=self.case["case_sha256"])

    def test_transaction_binds_case_packet_route_and_full_federation(self):
        tx = build_unified_shadow_transaction(
            self.case,
            self.packet,
            self.federation,
            self.route,
    ...
    def test_tampered_packet_is_rejected_at_transaction_boundary(self):
        ...
        with self.assertRaisesRegex(ShadowIntegrationError, "transaction_packet_hash_mismatch"):
            build_unified_shadow_transaction(
                self.case,
                packet,
                self.federation,
                self.route,
    ...
    def test_executor_cannot_be_enabled_even_with_rehashed_route(self):
        ...
        from tools.tradingos_shadow_integration import sha256_obj
